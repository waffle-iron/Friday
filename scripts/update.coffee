...
module.exports = (robot) ->
    ...
    robot.respond /update( yourself)?$/i, (msg) ->
        changes = false
        try
            msg.http("https://api.travis-ci.org/repos/codeiain/Friday")
            .get() (err, res,body) ->
                response = JSON.parse(body)
                if response.last_build_status == 0
                    msg.send "updating to build " + response.last_build_number
                    msg.send "git pull..."
                    ...
                    msg.send "Sorry I cant do that my last build faild"        